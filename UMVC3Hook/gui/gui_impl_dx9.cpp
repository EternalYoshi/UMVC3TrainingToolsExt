...
void GUIImplementationDX9::GUI_Process()
{
	...
	if (ms_bFirstDraw)
	{
		...
		Notifications->PushNotification("UMVC3Hook %s is running! Press %s to open the menu. Build date: %s\n", UMVC3HOOK_VERSION, eKeyboardMan::KeyToString(SettingsMgr->iHookMenuOpenKey), __DATE__);
		ms_bFirstDraw = false;
	}
