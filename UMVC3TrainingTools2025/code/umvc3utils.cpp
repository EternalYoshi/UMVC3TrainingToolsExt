...
static DWORD SingleWrite(DWORD64 AddressToWrite, int length, HANDLE hProcess, BYTE ByteToWrite, DWORD errorMessageID, LPSTR messageBuffer)
{
...
static DWORD MultiWrite(DWORD64 AddressToWrite, int length, HANDLE hProcess, BYTE ByteToWrite[], DWORD errorMessageID, LPSTR messageBuffer)
{
	if (!WriteProcessMemory(hProcess, (BYTE*)(AddressToWrite), ByteToWrite, length, NULL))
	{
...
static DWORD ProtectedSingleWrite(DWORD64 AddressToWrite, int length, HANDLE hProcess, BYTE ByteToWrite, DWORD errorMessageID, LPSTR messageBuffer, DWORD oldPageProtection)
{
	oldPageProtection = 0;
...
static DWORD ProtectedMultiWrite(DWORD64 AddressToWrite, int length, HANDLE hProcess, BYTE ByteToWrite[], DWORD errorMessageID, LPSTR messageBuffer, DWORD oldPageProtection)
{
	oldPageProtection = 0;
...
static void EndlessXFactorUpdate()
{
	GetPlayerData();
	float fli = 7000.0;
...
static bool InputDisplayValidator()
{

	long long vftaddress = 0;
...
static void LeftSideInputDisplay()
{
...
static void SetIndividualCharacterHealth()
{
...
static void ChangeFrankLevel(int FrankLevel)
{
	GetPlayerData();
	GetCharacterIDs();
...
static void ChangeWrightEvidence()
{
	GetPlayerData();
	GetCharacterIDs();
...
static int EvidenceNameToID(const char* name)
{
...
static void ChangeMODOKUnderstanding(int MODOKLOU)
{
	GetPlayerData();
	GetCharacterIDs();
...
static void SetDormSpellLevels()
{
	GetPlayerData();
	GetCharacterIDs();
...
static void EndlessInstallBoolUpdate()
{
	float flo = 21600.0;
	char FlagA = 0;
	GetPlayerData();
...
static void SetDeadpoolTeleport()
{
	GetPlayerData();
	GetCharacterIDs();
...
static void JammingToggleRestart()
{
...
static void SetGlobalPlayerSpeed(float GlobalSpeed)
{
...
static void GetActiveInstallData()
{
...
static void GetHitboxDataPart1()
{
...
static bool CheckIfDead(uintptr_t PlayerPtr)
{

	uintptr_t YHP = 0;
	uintptr_t RHP = 0;

	ReadProcessMemory(hProcess, (LPVOID*)(PlayerPtr + 0x1550), &YHP, sizeof(uintptr_t), 0);
	ReadProcessMemory(hProcess, (LPVOID*)(PlayerPtr + 0x1558), &RHP, sizeof(uintptr_t), 0);
